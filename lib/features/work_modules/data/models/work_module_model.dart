...
class WorkModuleModel {
  const WorkModuleModel({
    this.id,
    required this.name,
    this.description,
    this.active = true,
  ...
  factory WorkModuleModel.fromJson(Map<String, dynamic> json) {
    return WorkModuleModel(
      id: _readString(json, const ['id', '_id', 'uuid']),
      name: _readString(
            json,
            const ['name', 'title', 'workModuleName'],
          ) ??
          '',
  ...
  factory WorkModuleModel.fromEntity(WorkModule entity) {
    return WorkModuleModel(
      id: entity.id,
      name: entity.name,
